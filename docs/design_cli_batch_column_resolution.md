...
Placed alongside `Engine/IO/Csv/DataRowReader.cs`, which already depends on `nietras.SeparatedValues` — consistent with the existing dependency footprint of this namespace. Blank-name auto-naming (`ColumnN`) mirrors `App/Tui/Workers/Schema/Csv/IncrementalSchemaScanner.ReadColumnNames()` — CLI batch mode already goes through that method today via `IncrementalSchemaScanner`, so this preserves existing behavior for malformed headers rather than introducing new handling.
