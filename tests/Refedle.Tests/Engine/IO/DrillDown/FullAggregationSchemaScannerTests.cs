...
public sealed class FullAggregationSchemaScannerTests : IDisposable
{
    ...
    public void Scan_FormatOtherThanJsonArray_ThrowsUnreachable(DataFormat format)
    {
        // Arrange — only JsonArray is wired in Phase 3; JSON Lines is Phase 6.
        var file = CreateJsonArray("""{"orders":[{"id":1}]}""");

        // Act
        var act = () => FullAggregationSchemaScanner.Scan(file, format, KeyPath("orders"), CancellationToken.None);
